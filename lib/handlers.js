...
// ---- POST /api/manychat : ManyChat comment-automation webhook ----
//
// ManyChat flow overview:
//   1. Comment Growth Tool  →  keyword "distribution"
//   2. Instant DM: send the guide link
//   3. Quick-reply message: "Glad you grabbed it! Are you here for the
//      playbook, or looking for help implementing these strategies for your brand?"
//      • Button A → "The Playbook"  → (follow-up sequence inside ManyChat)
//      • Button B → "Help me implement" → External Request to this endpoint
//                   (event=qualified) + send booking link
//   4. Optin hook (optional): after step 2, External Request with event=optin
//      so you get an instant Discord ping for every new lead.
//
// How to configure ManyChat's External Request action:
//   URL  : https://<your-worker>.workers.dev/api/manychat?event=optin&secret={{MANYCHAT_WEBHOOK_SECRET}}
//   Method: POST
...
//       "email": "{{email}}"
//     }
//   (ManyChat field tokens like {{first name}} are filled in automatically.)
//
// Secrets to add in the Cloudflare dashboard (Settings → Variables → Secrets):
//   MANYCHAT_WEBHOOK_SECRET  — any random string; paste the same value into
//                              the ManyChat External Request URL above.
...
export async function handleManyChat(request, env) {
  ...
  // Verify the shared secret passed as ?secret=... in the URL.
  const secret = env.MANYCHAT_WEBHOOK_SECRET;
  if (secret) {
    const urlSecret = new URL(request.url).searchParams.get('secret');
    if (urlSecret !== secret) return json({ error: 'unauthorized' }, 401);
  }
  ...
  const event = new URL(request.url).searchParams.get('event') || 'optin';
  const name = cleanText(b.name || b.first_name || '');
  const ig = cleanText(b.ig || b.instagram_username || '');
  ...
  let content;
  if (event === 'qualified') {
    content = '🔥 **Hot ManyChat lead — wants help implementing!**\nThey commented "distribution", grabbed the guide, and said they want your help.\n' + (who || '_(no contact info captured yet)_');
  } else {
    content = '📥 **New ManyChat opt-in — grabbed the distribution guide.**\n' + (who || '_(no contact info captured yet)_');
  }
