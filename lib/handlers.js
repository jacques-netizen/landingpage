...
// ---- GET /api/notify-test : diagnose the Discord wiring from a browser ----
// Temporary helper. Open this URL in a browser; it reports whether the secret
// is set, whether it looks like a Discord webhook, and whether Discord accepts it.
export async function handleNotifyTest(request, env) {
  const hook = env.DISCORD_WEBHOOK_URL;
  const out = [];
  if (!hook) {
    out.push('❌ DISCORD_WEBHOOK_URL secret is NOT set on this Worker.');
    out.push('Fix: Cloudflare → landingpage → Settings → Variables and Secrets → add Secret named DISCORD_WEBHOOK_URL, then redeploy.');
    return new Response(out.join('\n'), { status: 200, headers: { 'content-type': 'text/plain; charset=utf-8' } });
  }
  out.push('✅ DISCORD_WEBHOOK_URL secret is set (length ' + hook.length + ').');
  const looksRight = /^https:\/\/(discord|discordapp)\.com\/api\/webhooks\//.test(hook.trim());
  out.push(looksRight
    ? '✅ It looks like a valid Discord webhook URL.'
    : '⚠️ It does NOT look like a Discord webhook URL (should start with https://discord.com/api/webhooks/). This is likely the problem.');
  try {
    const res = await fetch(hook.trim(), {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ content: '🧪 Test ping from your site — if you can see this in Discord, the wiring works! (Safe to ignore.)', allowed_mentions: { parse: [] } }),
    });
    out.push('Discord responded: HTTP ' + res.status + (res.ok ? ' ✅ — check your channel, the test message should be there.' : ' ❌ — Discord rejected the webhook URL.'));
    if (!res.ok) { const t = await res.text(); out.push('Discord said: ' + t.slice(0, 300)); }
  } catch (err) {
    out.push('❌ Error calling Discord: ' + String(err && err.message ? err.message : err));
  }
  return new Response(out.join('\n'), { status: 200, headers: { 'content-type': 'text/plain; charset=utf-8' } });
}
