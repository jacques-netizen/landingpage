...
export function deriveFromClips(clips, base, meta) {
  ...
  return {
    ...
    summary: {
      ...
      budgetSpent: Number(pick(meta.budgetSpent, bs.budgetSpent)) || 0,
...
// not placeholders. YouTube is derived from the video id (no network). TikTok
// uses its public oEmbed endpoint. Instagram / anything else falls back to a
// keyless preview service. Best-effort and bounded: any miss just leaves the
// clip without a thumb (the dashboard shows a tasteful placeholder).
...
async function fetchJSON(url, ms) {
  ...
  try {
    const r = await fetch(url, { signal: ctrl.signal, headers: { 'user-agent': 'Mozilla/5.0 (compatible; MaisonBot/1.0)' } });
    if (!r.ok) return null;
    return await r.json();
  } catch (e) { return null; } finally { clearTimeout(t); }
}

async function resolveOneThumb(clip) {
  ...
  // Instagram + anything else: microlink returns the post's preview image for
  // public posts (keyless). Falls through to a placeholder on failure.
...
async function resolveThumbnails(clips) {
  const queue = clips.slice();
  const run = async () => { while (queue.length) { const c = queue.shift(); if (c) { try { await resolveOneThumb(c); } catch (e) { /* skip */ } } } };
  const workers = Array.from({ length: Math.min(8, clips.length) }, run);
...
export async function handleCampaignImport(request, env) {
  if (!env.MEDIA) return json({ error: 'storage not configured' }, 500);
  if (request.method.toUpperCase() !== 'POST') return json({ error: 'method not allowed' }, 405);
  if (!isAuthed(request, env)) return json({ error: 'unauthorized' }, 401);
  ...
  if (!dryRun) await resolveThumbnails(clips);
