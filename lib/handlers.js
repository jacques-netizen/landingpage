...
function calContent(body) {
  ...
  const lines = [];
  var srcTag = cleanText(valOf(resp.utmSource) || (p.type && p.type !== 'discovery-call' ? p.type : '') || 'direct');
    lines.push(`**📍 Source:** ${srcTag}`);
  ...
  const shown = { name: 1, email: 1, guests: 1, utmSource: 1 };
  for (const k in resp) {
    if (shown[k]) continue;
    const r = resp[k];
...
// Public GET returns the live campaign JSON; authed POST overwrites it. Each
// client's campaign is its own JSON object in the R2 bucket, keyed by slug, so
// one Worker serves any number of clients at campaigns.maisondelites.com/<slug>.
const DEFAULT_CLIENT = 'itzdanielmichael';
const LEGACY_KEY = 'campaign-data.json';

// Sanitize a client slug: lowercase, [a-z0-9-], 1–40 chars. Defaults when absent.
function slugify(v) {
  const s = String(v == null ? '' : v).toLowerCase().replace(/[^a-z0-9-]/g, '').slice(0, 40);
  return s || DEFAULT_CLIENT;
}
// Read the client slug from a request's ?client= param.
function clientOf(request) {
  try { return slugify(new URL(request.url).searchParams.get('client')); }
  catch (e) { return DEFAULT_CLIENT; }
}
// R2 key for a client's campaign JSON.
function keyFor(slug) { return 'campaigns/' + slugify(slug) + '.json'; }
...
function sanitizeCampaign(input) {
  ...
  const clips = Array.isArray(d.clips) ? d.clips.slice(0, 120).map((c) => ({
    ...
    views: num(c && c.views),
    status: ['accepted', 'pending', 'rejected'].includes(c && c.status) ? c.status : '',
...
export async function handleCampaign(request, env) {
  ...
  const slug = clientOf(request);

  if (method === 'GET') {
    const data = await loadCampaign(env, slug);
  ...
  if (method === 'POST' || method === 'PUT') {
    ...
    // Merge over what's stored so a partial save (e.g. the admin's Save button
    // sending only name/budget) can never wipe published clips, creators or
    // platforms. A full POST still overrides every field, as before.
    const existing = await loadCampaign(env, slug);
    const merged = {
      ...existing,
      ...body,
      summary: { ...(existing.summary || {}), ...(body.summary || {}) },
    };
    const clean = sanitizeCampaign(merged);
    await env.MEDIA.put(keyFor(slug), JSON.stringify(clean), {
...
// ---- POST/DELETE /api/campaign/banner?client=<slug> : campaign banner image --
// Authed. A single image upload (banners are small — no multipart needed, unlike
// the video slot uploads above). Stored under a fixed per-client R2 key so
// re-uploading just replaces it, then the client's stored campaign is updated
// with a servable URL. Served publicly through the existing /v/ media handler
// (Range support, caching) — no new serving path needed.
const BANNER_PREFIX = 'campaign-banners/';
function bannerKey(slug) { return BANNER_PREFIX + slugify(slug); }

export async function handleCampaignBanner(request, env) {
  if (!env.MEDIA) return json({ error: 'storage not configured' }, 500);
  if (!isAuthed(request, env)) return json({ error: 'unauthorized' }, 401);
  const slug = clientOf(request);
  const key = bannerKey(slug);
  const method = request.method.toUpperCase();
  ...
  if (method === 'DELETE') {
    ...
    const clean = await saveBanner('');
    return json({ ok: true, deleted: true, banner: clean.banner });
  }
  if (method !== 'POST') return json({ error: 'method not allowed' }, 405);

  const contentType = request.headers.get('x-file-type') || request.headers.get('content-type') || 'image/jpeg';
  if (!/^image\//.test(contentType)) return json({ error: 'must be an image file' }, 400);
  let buf;
  try { buf = await request.arrayBuffer(); } catch (e) { return json({ error: 'could not read image data' }, 400); }
  if (!buf.byteLength) return json({ error: 'empty file' }, 400);
  if (buf.byteLength > 8 * 1024 * 1024) return json({ error: 'image too large — 8MB max' }, 400);

  await env.MEDIA.put(key, buf, { httpMetadata: { contentType } });
  const clean = await saveBanner('/v/' + key);
  return json({ ok: true, banner: clean.banner });
}

// ---- GET /api/campaigns : list every client campaign (authed) ---------------
// Powers the operator hub: one card per client with headline numbers, so you can
// click through to edit any of them. Reads each stored campaign under the
// campaigns/ prefix, plus the legacy default if it hasn't been re-published yet.
export async function handleCampaignList(request, env) {
  if (!isAuthed(request, env)) return json({ error: 'unauthorized' }, 401);
  if (!env.MEDIA) return json({ error: 'storage not configured' }, 500);
  const summaryOf = (slug, d) => ({
    slug,
    client: d.client || slug,
    campaign: d.campaign || '',
    banner: d.banner || '',
    views: (d.summary && d.summary.views) || 0,
    clips: (d.summary && d.summary.clips) || 0,
    creators: (d.summary && d.summary.creators) || 0,
    // Only count spend that is actually reported, so the hub's portfolio total
    // never implies money we can't evidence.
    spend: (d.summary && d.summary.spendKnown) ? (d.summary.budgetSpent || 0) : 0,
    updated: d.updated || null,
  });
  const out = [];
  const seen = new Set();
  try {
    const listed = await env.MEDIA.list({ prefix: 'campaigns/', limit: 1000 });
    for (const obj of (listed.objects || [])) {
      const m = obj.key.match(/^campaigns\/(.+)\.json$/);
      if (!m) continue;
      const slug = m[1];
      seen.add(slug);
      out.push(summaryOf(slug, await loadCampaign(env, slug)));
    }
  } catch (e) { /* fall through with whatever we have */ }
  // Include the legacy default client until it's re-published under its slug.
  if (!seen.has(DEFAULT_CLIENT)) {
    const legacy = await env.MEDIA.get(LEGACY_KEY);
    if (legacy) out.push(summaryOf(DEFAULT_CLIENT, await loadCampaign(env, DEFAULT_CLIENT)));
  }
  out.sort((a, b) => String(b.updated || '').localeCompare(String(a.updated || '')));
  return json({ clients: out });
}

// Load a client's saved campaign (or the shipped default) as a plain object.
// For the default client, fall back to the legacy single-file key so the
// original dashboard keeps working until it's re-published under its slug.
async function loadCampaign(env, slug) {
  slug = slugify(slug);
  let obj = await env.MEDIA.get(keyFor(slug));
  if (!obj && slug === DEFAULT_CLIENT) obj = await env.MEDIA.get(LEGACY_KEY);
  if (!obj) {
    // No data yet — a tasteful empty state named for this client.
    const d = defaultCampaign();
    if (slug !== DEFAULT_CLIENT) {
      d.client = slug.split('-').map((w) => (w ? w[0].toUpperCase() + w.slice(1) : w)).join(' ');
      d.campaign = 'Live Campaign';
    }
    return d;
  }
...
export function deriveFromClips(clips, base, meta) {
  ...
  for (const c of clips) {
    // Rejected clips still render as cards, but don't count toward the headline
    // views / clip count / platform split / leaderboard / spend.
  ...
  // Spend is NEVER estimated. It is either summed from a real per-clip
  // earnings/reward column in the CSV, or stated outright by the operator in the
  // admin (spendManual). Most Content Rewards exports carry $0.00 payouts, so the
  // manual figure is what actually drives the client-facing budget bar. A manual
  // value is an explicit override: it wins over the CSV and survives re-publishes.
  const spendManual = !!pick(meta.spendManual, bs.spendManual);
  const spendKnown = spendManual ? true : earned > 0;
  const budgetSpent = spendManual
    ? (Number(pick(meta.budgetSpent, bs.budgetSpent)) || 0)
    : (earned > 0 ? Math.round(earned * 100) / 100 : 0);

  return {
    client: pick(meta.client, base.client),
    campaign: pick(meta.campaign, base.campaign),
    tagline: pick(meta.tagline, base.tagline),
    banner: pick(meta.banner, base.banner),
    summary: {
      views,
      clips: countedClips,
...
async function resolveOneThumb(clip, env) {
  ...
  // Instagram: the public embed exposes the media image and works server-side
  // (no key needed). A dedicated Instagram API (RapidAPI) is an optional, more
  // robust fallback if the embed ever fails.
...
export async function resolveStoredThumbnails(env, slug) {
  slug = slugify(slug);
  try {
    const data = await loadCampaign(env, slug);
    ...
    if (!todo.length) {
      const have = clips.filter((c) => c && c.thumb).length;
      return { ok: true, resolved: 0, withThumbnail: have, total: clips.length };
    }
    // Resolve in small batches and SAVE after each one, so progress persists even
    // if a background run is cut short (otherwise a single save-at-the-end means
    // an interrupted run stores nothing).
    const BATCH = 6;
    for (let i = 0; i < todo.length; i += BATCH) {
      await resolveThumbnails(todo.slice(i, i + BATCH), env);
      await env.MEDIA.put(keyFor(slug), JSON.stringify(data), {
        httpMetadata: { contentType: 'application/json' },
      });
    }
    const withThumbnail = clips.filter((c) => c && c.thumb).length;
    return { ok: true, resolved: todo.filter((c) => c.thumb).length, withThumbnail, total: clips.length };
  } catch (e) { return { error: String(e && e.message ? e.message : e) }; }
}

// ---- POST /api/campaign/resolve : resolve thumbnails now (authed) -----------
// Runs resolution synchronously (with incremental saves) and returns counts, so
// it both fixes blank thumbnails on demand and reports exactly how many are
// stored. Safe to call repeatedly — it only works on clips still missing a thumb.
export async function handleResolveThumbs(request, env) {
  if (!isAuthed(request, env)) return json({ error: 'unauthorized' }, 401);
  if (!env.MEDIA) return json({ error: 'storage not configured' }, 500);
  const result = await resolveStoredThumbnails(env, clientOf(request));
  return json(result);
}
...
export async function handleThumbTest(request, env) {
  ...
  // ?stats=1 -> how many stored clips currently have a thumbnail (so we can see
  // whether resolution actually persisted).
  if (u.searchParams.get('stats')) {
    const data = await loadCampaign(env, clientOf(request));
    const clips = Array.isArray(data.clips) ? data.clips : [];
    const withThumb = clips.filter((c) => c && c.thumb);
    return json({
      totalClips: clips.length,
      withThumbnail: withThumb.length,
      withoutThumbnail: clips.length - withThumb.length,
      sampleThumbs: withThumb.slice(0, 3).map((c) => c.thumb),
      sampleMissing: clips.filter((c) => c && !c.thumb).slice(0, 3).map((c) => ({ platform: c.platform, url: c.url })),
    });
  }

  const target = u.searchParams.get('url') || '';
  if (!target) return json({ error: 'add ?url=<post url> or ?stats=1' }, 400);
  ...
  // Verify the Worker can actually fetch the found image (this is exactly what
  // /api/img does to re-serve it to the browser).
  if (trace.thumb) {
    const probe = { label: 'image-fetch' };
    try {
      const ctrl = new AbortController();
      const t = setTimeout(() => ctrl.abort(), 12000);
      const ir = await fetch(trace.thumb, { signal: ctrl.signal, headers: { 'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15', accept: 'image/*,*/*' } });
      clearTimeout(t);
      const buf = await ir.arrayBuffer();
      probe.httpStatus = ir.status;
      probe.contentType = ir.headers.get('content-type');
      probe.bytes = buf.byteLength;
      probe.imageLoadsServerSide = ir.ok && buf.byteLength > 500;
    } catch (e) { probe.error = String(e && e.message ? e.message : e); }
    trace.steps.push(probe);
    trace.displayVia = '/api/img?u=' + encodeURIComponent(trace.thumb);
  }
  return json(trace);
}

// ---- GET /api/img?u=<image url> : same-origin image proxy -------------------
// The browser can't load Instagram/TikTok CDN images directly (hotlink blocks),
// but the Worker can fetch them, so we re-serve them from our own origin. Host
// allowlisted so it isn't an open proxy.
export async function handleImageProxy(request, env) {
  const u = new URL(request.url);
  const target = u.searchParams.get('u') || '';
  if (!target) return new Response('missing u', { status: 400 });
  let host;
  try { host = new URL(target).hostname; } catch (e) { return new Response('bad url', { status: 400 }); }
  if (!/(?:cdninstagram|fbcdn|tiktokcdn|ytimg|akamaihd|twimg|licdn)\.com$|\.fna\.fbcdn\.net$|cdninstagram\.com$/i.test(host)
      && !/cdninstagram|fbcdn|tiktokcdn|ytimg|akamaihd|twimg|pinimg|licdn/i.test(host)) {
  ...
  try {
    ...
    const r = await fetch(target, {
      signal: ctrl.signal,
      headers: { 'user-agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15', accept: 'image/*,*/*' },
    });
    clearTimeout(t);
...
export async function handleCampaignImport(request, env, ctx) {
  ...
  await env.MEDIA.put(keyFor(slug), JSON.stringify(derived), {
    httpMetadata: { contentType: 'application/json' },
  });
  ...
  const finish = resolveStoredThumbnails(env, slug);
...
export async function pullCampaignCSV(env) {
  ...
  try {
    ...
    const slug = slugify(env.CAMPAIGN_CSV_CLIENT);
    const existing = await loadCampaign(env, slug);
