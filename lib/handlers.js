...
export async function handleNotify(request, env) {
  ...
  try {
    ...
    if (b && b.incomplete) incomplete = true;
  ...
  if (lead) {
    ...
    const header = incomplete
      ? '📅 **New discovery call booked** — ⚠️ they left before finishing the details, so some fields may be missing. Reach them via email / Calendly.'
