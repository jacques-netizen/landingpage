...
export function deriveFromClips(clips, base, meta) {
  ...
  // Spend, most accurate first:
  //   1. Sum a per-clip earnings/reward column if the CSV had one (matches the
  //      source platform's reported spend exactly).
  //   2. Otherwise views x payout rate (default $0.50 per 1,000 views).
  //   3. Otherwise a manually entered amount.
  const rate = Number(pick(meta.rate, bs.rate));
  let budgetSpent;
  if (earned > 0) budgetSpent = Math.round(earned * 100) / 100;
  else if (rate > 0) budgetSpent = Math.round(views / 1000 * rate * 100) / 100;
  else budgetSpent = Number(pick(meta.budgetSpent, bs.budgetSpent)) || 0;
  ...
  return {
    ...
    summary: {
      ...
      budgetSpent,
      rate: Number.isFinite(rate) && rate > 0 ? rate : (Number(bs.rate) || 0),
...
async function resolveOneThumb(clip) {
  ...
  // 1) OpenGraph image straight from the post page.
  const og = extractOgImage(await fetchText(url));
  ...
  // 3) Keyless preview service (last resort).
