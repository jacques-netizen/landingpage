...
const HEADERS = {
  ...
  earnings: ['earnings', 'earned', 'paid', 'amountpaid', 'paidamount', 'totalpaid', 'payout', 'payouts', 'reward', 'rewards', 'amount', 'revenue', 'spend', 'spent', 'cost', 'payment', 'commission'],
};
...
// Normalise a submission status to accepted | pending | rejected. Empty means
// the sheet has no status info (unknown) — treated as counted, shown untagged.
export function normStatus(v) {
  const s = String(v == null ? '' : v).trim().toLowerCase();
  if (!s) return '';
  if (/reject|denied|declin|flag|removed/.test(s)) return 'rejected';
  if (/approv|accept|live|paid|complete|verified/.test(s)) return 'accepted';
  return 'pending';
}
...
export function clipsFromCSV(text) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    out.push({
      ...
      status: idx.status != null ? normStatus(get('status')) : '',
      posted: get('posted'),
